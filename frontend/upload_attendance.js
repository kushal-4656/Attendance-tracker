...
function validateTotalStrength() {
  const total =
    parseInt(document.getElementById("total_strengthStrength").value) || 0;
  const present =
    parseInt(document.getElementById("present_countCount").value) || 0;
...
async function loadClasses() {
  ...
  try {
    const res = await fetch(
      `https://attendance-tracker-tvx5.onrender.com/classes/${faculty_id}`,
    );
    const data = await res.json();
