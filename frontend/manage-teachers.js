...
function displayTeachers() {
    ...
    if (pageTeachers.length === 0) {
        tbody.innerHTML = '<tr><td colspan="9" class="loading-row">No teachers found</td></tr>';
...
function viewTeacher(id) {
    ...
    if (teacher) {
        ...
        document.getElementById('viewId').textContent = `TCH${teacher.id.toString().padStart(3, '0')}`;
        ...
        document.getElementById('viewStatus').innerHTML = `<span class="status-badge status-${teacher.status.toLowerCase().replace(' ', '')}">${teacher.status}</span>`;
...
function exportData() {
    ...
    const headers = ['ID', 'Name', 'Email', 'Phone', 'Department', 'Status', 'Join Date'];
...
document.getElementById('filterDept').addEventListener('change', filterAndDisplay);